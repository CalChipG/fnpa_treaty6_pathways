    ...
    for fac in data_star[report_col].unique():                  # For each unique entry in the data where Location Data type is band, plotted with star symbol
        ...
        fig.add_trace(go.Scattermapbox(                         # Adds a point on the map based on long, lat, and name.
            ...
            mode='markers', marker=dict(
                symbol='star', size=10                          # Plotted as a start of size 20
...
with row11_1:
    ...
    st.markdown('''This map describes emitting facilities within a '''+ str(round(max_dist,2)) +''' hour of the selected band. Adjacent bands within a '''+ str(round(max_dist,2)) +''' hour radius are also displayed. All bands are displayed as stars. 
